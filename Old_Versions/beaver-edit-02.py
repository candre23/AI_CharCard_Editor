# 99% stolen from https://github.com/FaceDeer/pyqt_tavernai_character_editor
...
class AspectRatioLabel(QLabel):
    def __init__(self, pixmap):
        ...
        self.setStyleSheet("background-color: transparent;")  # Set transparent background

    def paintEvent(self, event):
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing) # Smooth out the edges
...
WARNING: Save your work first! Unsaved edits are discarded.""")
        ...
        self.token_count_label = QLabel("Token Count: 0", self)
        ...
        # self.vocab_selector.setSizePolicy(QSizePolicy.Maximum, QSizePolicy.Fixed)  # Shrink the dropdown

        self.refresh_button = QPushButton("Refresh Count", self)
        self.refresh_button.clicked.connect(self.updateTokenCount)
        # self.refresh_button.setSizePolicy(QSizePolicy.Maximum, QSizePolicy.Fixed)
        
        # Appearance settings widgets
        self.appearance_label = QLabel("Appearance:", self)
        self.appearance_label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)  # Right-justify
        ...
        self.token_layout.addSpacing(2)  # Add 2px spacing *before* the dropdown
    ...
    def changeFontSize(self, index):
        ...
        self.setFont(font)
        # Ensure all child widgets update their font size:
        for child in self.findChildren(QWidget):
            child.setFont(font)

        self.imageList.updateGeometry()  # Force re-layout to accomodate size changes


    def updateStack(self):
        self.splitter.widget(0).deleteLater()
        self.splitter.insertWidget(0, self.imageList.stack)
        self.updateTokenCount() #update token count when switching characters
    ...
    def calculateTokenCount(self, data):
        ...
        for key in ["name", "description", "personality", "scenario", "first_mes", "mes_example", "system_prompt", "post_history_instructions", "creator_notes"]:
            total_chars += len(data.get(key, ""))

        total_chars += sum(len(greeting) for greeting in data.get("alternate_greetings", []))
        total_chars += sum(len(tag) for tag in data.get("tags", []))  # Count tag characters as well
        total_chars += len(data.get("creator", ""))
        total_chars += len(data.get("character_version", ""))
        ...
        if "character_book" in data and "entries" in data["character_book"]:
            for entry in data["character_book"]["entries"]:
                ...
                total_chars += sum(len(key) for key in entry.get("keys", []))
                total_chars += len(entry.get("name", ""))
                total_chars += len(entry.get("comment", ""))
                total_chars += sum(len(key) for key in entry.get("secondary_keys", []))
